  ...
  if (!isAuthenticated) {
    return (
      <View className="flex-1 items-center justify-center bg-cooking-background">
        ...
        <Text className="text-lg text-cooking-text">登录查看我的菜谱</Text>
        <TouchableOpacity
  ...
  return (
    <View className="flex-1 bg-cooking-background">
      <View className="bg-white px-4 py-4 border-b border-gray-100">
        <Text className="text-xl font-bold text-cooking-text">我的菜谱</Text>
        <Text className="text-cooking-muted text-sm mt-1">
          共创建 {recipes.length} 道菜谱
        </Text>
      </View>
      ...
      <ScrollView className="flex-1 px-4 py-4">
        {recipes.length === 0 ? (
          <View className="py-20 items-center">
            <Text className="text-5xl mb-4">👨‍🍳</Text>
            <Text className="text-cooking-muted">还没有创建菜谱</Text>
            <TouchableOpacity
              className="bg-cooking-main px-6 py-2 rounded-xl mt-4"
              onPress={() => router.push('/(tabs)/camera')}
            >
              <Text className="text-white font-medium">创建菜谱</Text>
            </TouchableOpacity>
          </View>
        ) : (
          recipes.map((recipe: any) => (
            ...
            >
              ...
              <View className="flex-1 ml-3 justify-center">
                ...
                <Text className="text-cooking-muted text-sm mt-1">
                  {recipe.cuisine?.name || ''} · 👁 {recipe.viewCount || 0}次浏览
                </Text>
              </View>
            </TouchableOpacity>
          ))
